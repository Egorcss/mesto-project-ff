...
function handleProfileFormSubmit(evt) {
    ...
    .then(closeModal(popupRedactionProfile))
...
function handleAvatarFormSubmit(evt) {
    ...
    // Вызываю функцию с переменной, значение в которой ссылка на новую аватарку
    redactionMyProfileAvatar(profileAvatarImageLink.value)
    .then(data => {
    ...
    .then(closeModal(popupRedactionAvatar))
...
function handleAddCardOnPage(evt) {
    ...
    // просто передать titleInputAddCard.value и linkInputAddCard.value не получится
    const newCard = {
        name: titleInputAddCard.value, // Заголовок для новой карточки
        link: linkInputAddCard.value // Ссылка для новой карточки
    };    
    ...
    .then(closeModal(popupAddNewCard)) // Закрываю после добавления карточки на страницу модальное окно и очищаю форму методом reset
